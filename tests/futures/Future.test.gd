...
func it_all_only_resolves_after_all_promises_done():
    ...
    expect(future1.fulfilled).to.equal(0.1, "Future 1 done")
    expect(future2.fulfilled).to.NOT.equal(0.2,"Future 2 NOT yet done")
    expect(future3.fulfilled).to.NOT.equal(0.3,"Future 1 NOT yet done")
    ...
    expect(future1.fulfilled).to.equal(0.1)
    expect(future2.fulfilled).to.equal(0.2)
    expect(future3.fulfilled).to.NOT.equal(0.3)
    expect(future_all.fulfilled).to.equal(null)
    
    await future3.completed()
    expect(future1.fulfilled).to.equal(0.1)
    expect(future2.fulfilled).to.equal(0.2)
    expect(future3.fulfilled).to.equal(0.3)
    expect(future_all.fulfilled).to.equal([0.1,0.2,0.3])
    
func create_wait_future(time:float) -> Future:
    var future = Future.new()
    get_tree()\
        .create_timer(time)\
        .timeout\

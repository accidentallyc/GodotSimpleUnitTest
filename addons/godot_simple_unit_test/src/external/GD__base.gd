...
## Original floor to use in GD__.floor
...
class Internal:
	...
	func get_iteratee(callable: Callable):
		...
		match callable:
			GD__.take: return iteratee_drop_second_arg(GD__.take)
			GD__.every: return iteratee_drop_second_arg(GD__.every)
		return callable
	...
	func base_difference_by(array_left, array_right, iteratee = GD__.identity): 
	...
	func keyed_iterable(thing, from_index = 0):
		if GD__.is_array_like(thing) or GD__.is_string(thing):
...
static func _is_collection(item):
	return GD__.is_array_like(item) or item is Dictionary or GD__.is_string(item)
